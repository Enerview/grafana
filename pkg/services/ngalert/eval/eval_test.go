...
func TestConditionEvaluator(t *testing.T) {
	t.Run("when pipeline contains Hysteresis command", func(t *testing.T) {
		t.Run("and loaded metrics are provided", func(t *testing.T) {
			expected := map[data.Fingerprint]struct{}{
				data.Fingerprint(1): {},
				data.Fingerprint(2): {},
			}
			evaluated := false
			e := conditionEvaluator{
				pipeline: expr.DataPipeline{
					&expr.CMDNode{Command: &expr.HysteresisCommand{}},
					&expr.CMDNode{Command: &expr.ReduceCommand{}},
					&expr.CMDNode{Command: &expr.HysteresisCommand{}},
				},
				expressionService: &fakeExpressionService{
					hook: func(ctx context.Context, now time.Time, pipeline expr.DataPipeline) (*backend.QueryDataResponse, error) {
						cmds := expr.GetCommandsFromPipeline[*expr.HysteresisCommand](pipeline)
						assert.Len(t, cmds, 2)
						for i, cmd := range cmds {
							assert.EqualValuesf(t, expected, cmd.LoadedDimensions, "Command at index %d was not patched", i)
						}
						evaluated = true
						return &backend.QueryDataResponse{}, nil
					},
				},
				loadedMetricsReader: &FakeLoadedMetricsReader{fingerprints: expected},
			}

			t.Run("EvaluateRaw", func(t *testing.T) {
				evaluated = false
				_, err := e.EvaluateRaw(context.Background(), time.Now())
				require.NoError(t, err)

				t.Run("should patch commands", func(t *testing.T) {
					require.Truef(t, evaluated, "hook was not evaluated")
				})
				t.Run("should clean up after evaluation", func(t *testing.T) {
					cmds := expr.GetCommandsFromPipeline[*expr.HysteresisCommand](e.pipeline)
					assert.Len(t, cmds, 2)
					for i, cmd := range cmds {
						assert.Empty(t, cmd.LoadedDimensions, "Command at index %d was not cleaned up", i)
					}
				})
			})
			...
			t.Run("Evaluate", func(t *testing.T) {
				evaluated = false
				_, err := e.Evaluate(context.Background(), time.Now())
				require.NoError(t, err)
