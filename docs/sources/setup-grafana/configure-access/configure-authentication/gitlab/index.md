...
1. Set `validate_id_token` to `true` in the `[auth.gitlab]` section of the Grafana configuration file.
2. Configure `jwk_set_url` with the URL of your GitLab instance's JSON Web Key Set (JWKS) endpoint. This endpoint provides the public keys used to verify JWT signatures.
...
| `default_team`               | No       | No                 | Name of an existing Grafana team that users authenticating with this provider are added to on every login. The team must already exist in the organization the user signs in to. Overrides the global `default_team` in the `[users]` section rather than adding to it. Existing members are never removed, so clearing this setting leaves previously assigned memberships in place.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |                                      |
